...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 100 ????? 0000011", lw     , I, R(rd) = Mr(src1+imm,1)); //
  INSTPAT("0000000 ????? ????? 001 ????? 0010011", slli   , I, R(rd) = src1 << (imm & 0x1f)); //

  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai   , I, R(rd) = ((src1 >> (imm & 0x1f))|((src1>>31)<<31)));
  ...
  // INSTPAT("??????? ????? ????? 110 ????? 0000011", lb     , I, R(rd) = Mr(src1+imm,1));
  // INSTPAT("??????? ????? ????? 011 ????? 0000011", ld     , I, R(rd) = Mr(src1+imm,8));
  // INSTPAT("??????? ????? ????? 100 ????? 0000011", lwu    , I, R(rd) = Mr(src1+imm,4));
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 101 ????? 1100011",bge, B, if(src1 >= src2) s->dnpc = s->pc + imm); //1
