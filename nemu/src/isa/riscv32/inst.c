...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 000 00000 1100111", ret    , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm));
  INSTPAT("??????? ????? ????? ??? 00000 1101111", j      , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm);
